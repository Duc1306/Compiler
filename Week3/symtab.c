...
Object* lookupObject(char *name) {
  ...
  /* 1. Tìm từ scope hiện tại ra outer scope */
  while (scope != NULL) {
  ...
  /* 2. Tìm global object list (WRITEI, READI, WRITELN...) */
  Object *gobj = findObject(symtab->globalObjectList, name);
...
void declareObject(Object* obj) {
  ...
  Object *dup = findObject(symtab->currentScope->objList, obj->name);
  if (dup != NULL) {
    error(ERR_DUPLICATE_IDENT, 0, 0);   // Bạn có thể dùng line/col của currentToken nếu muốn chính xác
  }

  /* Nếu là PARAMETER → thêm vào paramList của owner */
